...
class VAE(Model):
    ...
    def calculate_loss(self, x, beta=1., average=False):
        ...
        if self.args.adv is True:
            ...
            loss += self.args.ksi * (FI -self.args.F).abs()
    ...
    def psnr(self,x):
        ...
        x_mean, x_logvar, z_q, z_q_mean, z_q_logvar = self.forward(x)
        data = Variable(x.data,volatile=True)
        mean = Variable(x_mean.data,volatile=True)
        mse = criterionMSE(data, mean)
    ...
    def calculate_dist(self, X, dir, mode='test'):
        # set auxiliary variables for number of training and test sets
        x_mean, x_logvar, z_q, z_q_mean, z_q_logvar = self.forward(X)
        mean = torch.mean(z_q,dim=0)
        dist = torch.norm((z_q - mean)/z_q.size()[1],p=2,dim=1)
        plot_latent_histogram(dist.cpu().data.numpy(), dir, mode)
